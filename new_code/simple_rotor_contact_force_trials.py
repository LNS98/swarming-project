...
def main():
    ...
    noise_list = list(np.linspace(0, 2*math.pi, num = 20))

    # number of repeats to take
    repeats = 4 * 5

    # make 1 complete run of the system
    poolie = Pool(processes = 4)

    data = poolie.map(variation, [i for i in range(repeats)])
    ...
    ave_values = np.mean(data_np, axis = 0)
    std_values = np.std(data_np, axis = 0)
    std_err_values = std_values / np.sqrt(repeats)


    df = pd.DataFrame({"noise list": noise_list, "v_a": ave_values, "std":std_values, "std error": std_err_values})
    df.to_csv("./results_phase_transitions/kNN/al_vs_noise_N_40.csv", index = False)

    # phase_transition(ave_values, noise_list)
...
def one_run(plot = False):
    ...
    positions_polygons = [polygon([L/2, L/2], L*0.1, math.pi /2, 15) for i in range(M)]
    ...
    if plot == True:
        show_path_2D(0, U, pos_part_over_t, pos_poly_over_t, clear = False)
...
def update_acceleration(position_particle, velocity_particle, position_particles, velocity_particles, positions_obj, polygons):
    ...
    gamma * allignment_force(position_particle, velocity_particle, position_particles, velocity_particles)
    + error_force(velocity_particle)) / mass_par
...
def help():
    ...
    poly_1 = polygon([L/2, L/2], 0.629867564929, 3.85395341416, 7)
    poly_2 = polygon([L/2, L/2], 0.671308301671, 3.93260821651, 10)
    poly_3 = polygon([L/2, L/2], 0.656872186747, 4.84245309456, 7)
