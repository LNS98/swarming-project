...
b = 0.629867564929 # outer size of radius of object
...
noise = 2  # noise added to the velocity
...
def one_run(plot = False):
    ...
    positions_polygons = [polygon([L/2, L/2], 0.5, math.pi /2, 8) for i in range(M)]
    ...
    # get the average angualr velocity for the last x runs
    # ave_ang_vel = np.mean(np.array(ang_vel_obj_over_t[U - 100: U]))

    # plot the movment of the particles if plot is set to true
    if plot == True:
        show_path_2D(0, U, pos_part_over_t, pos_poly_over_t, clear = True)

    return ang_vel_obj_over_t
...
def variation():
    ...
    for no in noise_list:
        ...
        path = "./alignment/"
        filename = "align_noise_variation.csv".format(noise)

        repeats  = 24 * 20
        # make 1 complete run of the system
        poolie = Pool(processes = 24)

        data = poolie.map(one_run, [False for i in range(repeats)])
        ...
        # get the ensemble  averages
        ave = np.mean(data_np, axis = 0)
        s_d = np.std(data_np, axis = 0)
        s_d_e = s_d / (repeats ** (1/ 2))

        # get the time averages
        ave_w = np.mean(ave[U - 1: U])
        s_d_w = np.std(ave)
        s_d_e_w = s_d_w / ((U - 1) ** (1/ 2))

        ave_list.append(ave_w)
        s_d_list.append(s_d_w)
        s_d_e_list.append(s_d_e_w)

        print("average is: {} with SD of {}".format(ave_w, s_d_w))

    df = pd.DataFrame(data = {"noise": noise_list, "average": ave_list, "std": s_d_list, "std error": s_d_e_list})
...
def error_force(incoming_velocity):
    ...
    # create a vector equal to the mag of the incoming velocity and with a random orientation
    incoming_velocity = np.array(incoming_velocity)
    acc_angle = np.arctan2(incoming_velocity[1], incoming_velocity[0])
    mag = np.sqrt(incoming_velocity.dot(incoming_velocity))
    acc_angle += random.uniform(- noise / 2, noise / 2)
    random_vector = np.array(angle_to_xy(mag, acc_angle))

    # force = -incoming velocity + radom vector
    force = random_vector - incoming_velocity
...
def help():
    ...
    # poly_1 = polygon([L/2, L/2], 0.629867564929, 4.84245309456, 7)
    # poly_1 = polygon([L/2, L/2], 0.671308301671, math.pi/9, 9)
    poly_1 = polygon([L/2, L/2],L*0.2 , 4.84245309456, 7)

    points_to_plot = np.array(poly_1)
