...
beta = 1e5 # stregnth of the force due to the objects on the particles
...
def one_run(plot = False):
    ...
    positions_polygons = [polygon([L/2, L/2], L * 0.05, L * 0.15, -math.pi/6, 7) for i in range(M)]
...
def contact_force_particle(polygon, position_obj, velocity_particle, position_particle):
        ...
        if poly_poly.contains(point):
            global stop
            stop = True
            print("Inside!!!")
            return np.array([-1e15 * velocity_particle[0], -1e15 * velocity_particle[1]])
